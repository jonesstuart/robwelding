 ...
 */
if ( ! class_exists( 'constructobuild_Theme_Info' ) ) {
    class constructobuild_Theme_Info{
        ...
        public function __construct( $config ) {
            ...
            add_action( 'wp_ajax_constructobuild_theme_info_update_recommended_action', array( $this, 'update_recommended_action_callback' ) );
        }
        ...
        public function prepare_class() {
            ...
            $this->page_title    = $this->theme_name . esc_html__( ' Info', 'constructionpro-lite' );
            $this->menu_title    = $this->theme_name . esc_html__( ' Theme', 'constructionpro-lite' );
        ...
        function bk_admin_menu() {
            ...
            if ( ! empty( $this->page_title ) && ! empty( $this->menu_title ) ) {
                ...
                add_theme_page( $this->page_title, $menu_title, 'edit_theme_options', $this->theme_slug . '-info', array(
                    $this,
                    'constructobuild_theme_info_screen',
                ) );
            }
        }
         ...
         */
        public function constructobuild_theme_info_screen() {
            ...
            if (
                ...
                echo '<a href="' . esc_url( 'https://templatehouse.net/themes/constructo-construction-wordpress-theme/' ).'" target="_blank">';
        ...
        public function recommended_actions() {
            ...
            if ( ! empty( $recommended_actions ) ) {
                        ...
                        if ( ! empty( $plugin_slug ) ) {
                            ...
                            switch ( $active['needs'] ) {
                                ...
                                case 'install':
                                    ...
                                    $label = esc_html__( 'Install', 'constructionpro-lite' );
                                    break;

                                case 'activate':
                                    $class = 'activate-now button button-primary';
                                    $label = esc_html__( 'Activate', 'constructionpro-lite' );

                                    break;

                                case 'deactivate':
                                    $class = 'deactivate-now button';
                                    $label = esc_html__( 'Deactivate', 'constructionpro-lite' );
                ...
                if ( $hooray ){
                    echo '<span class="hooray">' . esc_html__( 'Hooray! There are no recommended actions for you right now.', 'constructionpro-lite' ) . '</span>';
                    echo '<a data-action="reset" id="reset" class="reset-all" href="#">'.esc_html__('Show All Recommended Actions', 'constructionpro-lite').'</a>';
                }
                echo '</div>';
            }
        }
        ...
        public function call_plugin_api( $slug ) {
            ...
            if ( false === ( $call_api = get_transient( 'constructobuild_theme_info_plugin_information_transient_' . $slug ) ) ) {
                ...
                set_transient( 'constructobuild_theme_info_plugin_information_transient_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
            }
                        ...
                        if( !empty( $screenshot) ){
                            ...
                            if ( ! empty( $title ) ) {
                                ...
                                if ( ! empty( $download_link ) ) {
                                    echo '<a href="' . esc_url( $download_link ) . '" class="button button-primary install right">' . esc_html__( 'Download', 'constructionpro-lite' ) . '</a>';
                                }
                                if ( ! empty( $preview_link ) ) {
                                    echo '<a class="button button-secondary preview right" target="_blank" href="' . $preview_link . '">' . esc_html__( 'Live Preview', 'constructionpro-lite' ). '</a>';
                                }
                                echo '</div>';
                                echo '</div>';
                            }
        ...
        public function free_pro() {
            ...
            if ( ! empty( $free_pro ) ) {
                ...
                if ( ! empty( $free_pro ) && is_array( $free_pro ) ) {
                    ...
                    echo '<thead>';
                    echo '<tr>';
                    echo '<th>' . esc_html__( 'Theme Features','constructionpro-lite' ) . '</th>';
                    echo '<th>' . esc_html__( 'Free','constructionpro-lite' ) . '</th>';
                    echo '<th>' . esc_html__( 'Pro (Premium)','constructionpro-lite' ) . '</th>';
                    echo '</tr>';
                    echo '</thead>';
                    echo '<tbody>';
                    ...
                    echo '<td></td>';
                    echo '<td></td>';
                    echo '<td><a href="' . esc_url( 'https://templatehouse.net/themes/constructo-construction-wordpress-theme/') .'" target="_blank" class="button button-primary button-hero">Buy Pro</a></td>';
                    echo '</tr>';
                    echo '</tbody>';
                    echo '</table>';
                    echo '</div>';
        ...
        public function style_and_scripts( $hook_suffix ) {
            ...
            if ( 'appearance_page_' . $this->theme_slug . '-info' == $hook_suffix ) {
                ...
                $count = $this->count_recommended_actions();
                wp_localize_script( 'bk-theme-info-js', 'constructobuild_theme_info_object', array(
...
$config = array(
    ...
    // Main welcome title
    'info_title' => sprintf( esc_html__( 'Welcome to %s - ', 'constructionpro-lite' ), $theme_name_config ),

    // Main welcome content
    'info_content' => sprintf( esc_html__( '%s is now installed and ready to use. We hope the following information will help and you enjoy using it!', 'constructionpro-lite' ), '<b>'.$theme_name_config.'</b>' ),
    ...
    'quick_links' => array(
        'theme_url'  => array(
            'text' => __( 'Theme Info', 'constructionpro-lite' ),
            'url' => esc_url( 'https://templatehouse.net/themes/constructo-construction-wordpress-theme/' )
        ),
        'pro_url'  => array(
            'text' => __( 'Buy Pro', 'constructionpro-lite' ),
            'url' => esc_url( 'https://templatehouse.net/themes/constructo-construction-wordpress-theme/' )
        ),
    ),

    'tabs' => array(
        'getting_started'      => esc_html__( 'Getting Started', 'constructionpro-lite' ),
        'free_pro'             => esc_html__( 'Compare Free Vs Pro', 'constructionpro-lite' ),
        'recommended_plugins'  => esc_html__( 'Recommended Plugins', 'constructionpro-lite' ),
    ...
    'gs_steps' => array(
        ...
        'two' => array(
            'title' => esc_html__( 'Pre-built Demos', 'constructionpro-lite' ),
            'desc' => esc_html__( 'We recommend utilizing our collection of pre-designed starter site templates as an efficient and convenient way to begin your project.', 'constructionpro-lite' ),
            'button_label' => esc_html__( 'Browse Demos', 'constructionpro-lite' ),
            'button_link' => esc_url( 'https://demo.templatehouse.net/constructo/' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        ...
        'seven' => array(
            'title' => esc_html__( 'Need WordPress Theme Developemnt or Customization?', 'constructionpro-lite' ),
            'desc' => esc_html__( 'We Create Beautiful & SEO Friendly WordPress Websites. Theme Customization to Custom WordPress Plugin Development.', 'constructionpro-lite' ),
            'button_label' => esc_html__( 'Contact Now', 'constructionpro-lite' ),
        ...
        'five' => array(
            'title' => esc_html__( 'Love Our Product?', 'constructionpro-lite' ),
            'desc' => esc_html__( 'We would greatly appreciate your motivation through a 5-star rating on our profile.', 'constructionpro-lite' ),
            'button_label' => esc_html__( 'Rate Us', 'constructionpro-lite' ),
            'button_link' => esc_url( 'https://wordpress.org/support/theme/' .$theme_name_config_url. '/reviews' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'six' => array(
            'title' => esc_html__( 'Have Question? Ask Us, Now!', 'constructionpro-lite' ),
            'desc' => esc_html__( 'We are committed to resolving any issues that may arise to ensure a seamless experience while using our theme.', 'constructionpro-lite' ),
            'button_label' => esc_html__( 'Get Support', 'constructionpro-lite' ),
    ...
    'free_pro' => array(
        array(
            'desc'=> __( 'Access to All Starter Sites', 'constructionpro-lite' ),
            'free' => __( 'no','constructionpro-lite' ),
            'pro'  => __( 'yes','constructionpro-lite' ),
        ), 
        array(
            'desc'=> __( 'Access to All Templates & Patterns', 'constructionpro-lite' ),
            'free' => __( 'no','constructionpro-lite' ),
            'pro'  => __( 'yes','constructionpro-lite' ),
        ),
        array(
            'desc'=> __( 'Work with the Page Builders', 'constructionpro-lite' ),
            'free' => __( 'no','constructionpro-lite' ),
            'pro'  => __( 'yes','constructionpro-lite' ),
        ),
        array(
            'desc'=> __( 'Creative & Unique Blocks', 'constructionpro-lite' ),
            'free' => __( 'no','constructionpro-lite' ),
            'pro'  => __( 'yes','constructionpro-lite' ),
        ),
        array(
            'desc'=> __( 'Elementor Ready', 'constructionpro-lite' ),
            'free' => __( 'no','constructionpro-lite' ),
            'pro'  => __( 'yes','constructionpro-lite' ),
        ),
        array(
            'desc'=> __( 'Priority Updates', 'constructionpro-lite' ),
            'free' => __( 'no','constructionpro-lite' ),
            'pro'  => __( 'yes','constructionpro-lite' ),
        ),
        array(
            'desc'=> __( 'Priority Support', 'constructionpro-lite' ),
            'free' => __( 'no','constructionpro-lite' ),
            'pro'  => __( 'yes','constructionpro-lite' ),
        ),
        array(
            'desc'=> __( 'Full Site Editing', 'constructionpro-lite' ),
            'free' => __( 'yes','constructionpro-lite' ),
            'pro'  => __( 'yes','constructionpro-lite' ),
        ),
        array(
            'desc'=> __( 'Customize Everything', 'constructionpro-lite' ),
            'free' => __( 'yes','constructionpro-lite' ),
            'pro'  => __( 'yes','constructionpro-lite' ),
        ),
        array(
            'desc'=> __( 'Lighting Fast', 'constructionpro-lite' ),
            'free' => __( 'yes','constructionpro-lite' ),
            'pro'  => __( 'yes','constructionpro-lite' ),
        ),
        array(
            'desc'=> __( 'One-click Demo Installer', 'constructionpro-lite' ),
            'free' => __( 'no','constructionpro-lite' ),
            'pro'  => __( 'yes','constructionpro-lite' ),
        ),
        array(
            'desc'=> __( 'Fully Responsive', 'constructionpro-lite' ),
            'free' => __( 'yes','constructionpro-lite' ),
            'pro'  => __( 'yes','constructionpro-lite' ),
        ),
        array(
            'desc'=> __( 'Clean & Optimized Code', 'constructionpro-lite' ),
            'free' => __( 'yes','constructionpro-lite' ),
            'pro'  => __( 'yes','constructionpro-lite' ),
        ),
        array(
            'desc'=> __( 'Multiple Site Styles', 'constructionpro-lite' ),
            'free' => __( 'yes','constructionpro-lite' ),
            'pro'  => __( 'yes','constructionpro-lite' ),
        ),
        array(
            'desc'=> __( 'Global Settings', 'constructionpro-lite' ),
            'free' => __( 'yes','constructionpro-lite' ),
            'pro'  => __( 'yes','constructionpro-lite' ),
        ),
        array(
            'desc'=> __( 'CSS Animation', 'constructionpro-lite' ),
            'free' => __( 'yes','constructionpro-lite' ),
            'pro'  => __( 'yes','constructionpro-lite' ),
        ),
        array(
            'desc'=> __( 'Gutenberg Ready', 'constructionpro-lite' ),
            'free' => __( 'yes','constructionpro-lite' ),
            'pro'  => __( 'yes','constructionpro-lite' ),
        ),
        array(
            'desc'=> __( 'eCommerce Ready', 'constructionpro-lite' ),
            'free' => __( 'yes','constructionpro-lite' ),
            'pro'  => __( 'yes','constructionpro-lite' ),
        ),
        array(
            'desc'=> __( 'SEO Optimized', 'constructionpro-lite' ),
            'free' => __( 'yes','constructionpro-lite' ),
            'pro'  => __( 'yes','constructionpro-lite' ),
        ),
        array(
            'desc'=> __( 'Multilingual Compatibility', 'constructionpro-lite' ),
            'free' => __( 'yes','constructionpro-lite' ),
            'pro'  => __( 'yes','constructionpro-lite' ),
        ),
        array(
            'desc'=> __( 'RTL/LTR Language Support', 'constructionpro-lite' ),
            'free' => __( 'yes','constructionpro-lite' ),
            'pro'  => __( 'yes','constructionpro-lite' ),
        ),
    ),
...
return new constructobuild_Theme_Info( $config );